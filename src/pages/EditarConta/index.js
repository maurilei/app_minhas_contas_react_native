...
export default function EditarConta({route}) {
  const {idc, desc, val, venci, situ} = route.params;
  // return console.log(route.params.situ);
  const navigation = useNavigation();

  let ddd = new Date();
  const idConta = route.params.idc;
  const [isDatePickerVisible, setDatePickerVisibility] = useState(false);
  const [descricao, setDescricao] = useState(route.params.desc);
  const [valorInput, setvalorInput] = useState(route.params.val);
  const [valor, setvalor] = useState(route.params.val);
  const [vencimento, setVencimento] = useState(
    moment(route.params.venci).format('DD/MM/YYYY'),
  );
  const [vencData, setVencimentoData] = useState(
    moment(ddd).format('YYYY-MM-DD'),
  );
  const [checkBoxChecked, setCheckBoxChecked] = useState(route.params.situ);
  ...
  const alertSalvar = () => {
    if (descricao == null || descricao.length < 3) {
      ...
      Alert.alert(
        'Salvar',
        'Deseja Salvar A Conta Editada?', // <- this part is optional, you can pass an empty string
        [
          {
            text: 'Cancel',
  ...
  return (
    <SafeAreaView style={styles.container}>
      ...
      <View style={styles.topo}>
        <Text style={styles.topoTexto}>Editar Conta</Text>
      </View>
